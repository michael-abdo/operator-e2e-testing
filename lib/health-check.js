...
class HealthCheckSystem {
    constructor(options = {}) {
        ...
        this.skipSystemChecks = options.skipSystemChecks || false; // Option to bypass system resource checks
    }
    ...
    async checkSystemHealth() {
        // Option to completely skip system checks for CI/CD or constrained environments
        if (this.skipSystemChecks) {
            return {
                healthy: true,
                message: 'System checks skipped',
        ...
        try {
            ...
            // Much more lenient memory thresholds
            // Only fail if REALLY low (under 10MB is basically unusable)
            if (freeMB < 10) {
                return {
                    healthy: false,
            ...
            // Much more lenient CPU load threshold
            // Modern systems can handle high loads, especially with multiple cores
            // Only fail if EXTREMELY high (>50 is basically system hanging)
            if (loads[0] > 50) {
                return {
                    healthy: false,
                    message: 'Extremely high CPU load',
                    details: { loadAverage: loads[0] },
                    recovery: 'Wait for system load to decrease'
                };
            } else if (loads[0] > 20) {
                // Just a warning, don't fail
                console.log(`⚠️  High CPU load warning: ${loads[0]}`);
            }
