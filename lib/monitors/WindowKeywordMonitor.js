...
class WindowKeywordMonitor extends ChainKeywordMonitor {
    constructor(config) {
        ...
        this.lastDetectedContent = ''; // Track the exact content when we last detected TASK_FINISHED
    ...
    async checkOutput() {
        ...
        try {
            ...
            if (hasNewContent) {
                // Content changed - only reset if significantly different (avoid minor changes)
    ...
    /**
     * Override detectKeywordInOutput to add cooldown mechanism
     */
    async detectKeywordInOutput(keyword) {
        ...
        // Check if we're seeing the same content as last detection
        if (this.outputBuffer === this.lastDetectedContent && this.lastDetectedContent !== '') {
            console.log(`🔄 Same content as last detection, skipping`);
            return false;
        }
        ...
        if (detected) {
            // Record detection time and content
