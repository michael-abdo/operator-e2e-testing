...
class WindowKeywordMonitor extends ChainKeywordMonitor {
    ...
    async readFromInstance() {
        try {
            // Only capture last 20 lines where TASK_FINISHED would appear
            // This avoids buffer management issues with large outputs (13k+ chars)
            const { stdout } = await execAsync(
                `tmux capture-pane -t ${this.windowIndex} -p -S -20`
            );
            return stdout;
        } catch (error) {
            this.emit('error', {
