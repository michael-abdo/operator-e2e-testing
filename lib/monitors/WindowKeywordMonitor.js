...
class WindowKeywordMonitor extends ChainKeywordMonitor {
    ...
    async readFromInstance() {
        try {
            // Capture entire pane content and get the actual last 20 lines
            // This ensures we get the true tail where TASK_FINISHED appears
            const { stdout } = await execAsync(
                `tmux capture-pane -t ${this.windowIndex} -p | tail -20`
            );
            return stdout;
        } catch (error) {
            this.emit('error', {
