...
class WindowKeywordMonitor extends ChainKeywordMonitor {
    constructor(config) {
        ...
        this.projectContext = config.projectContext; // Store project context for isolation
        
        // Create logger for this window
        this.logger = createLogger(`Window-${this.windowIndex}`);
    }
    ...
    async checkOutput() {
        ...
        this.logger.info(`POLL #${this.pollCount} (${elapsed}s) - Looking for: "${this.currentKeyword}"`);
    ...
    async sendToInstance(text) {
        try {
            ...
            // Use RetryUtility for consistent retry behavior
            return await RetryUtility.forOperatorCommunication(
                sendOperation,
    ...
    async executeChainAction(chainConfig) {
        ...
        // Emit keyword_detected event that E2E system expects
        ...
        // Record this execution using unified state manager
        const executionData = {
            keyword,
            instruction,
            chainIndex,
        ...
        this.executedChains.push(executionData);
        
        // Track in state manager
        stateManager.trackExecution(`window-${this.windowIndex}`, {
