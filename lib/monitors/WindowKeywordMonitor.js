...
class WindowKeywordMonitor extends ChainKeywordMonitor {
    constructor(config) {
        ...
        this.detectionCooldown = 60000; // 60 second cooldown after detection
    ...
    /**
     * Override detectKeywordInOutput with simple 60-second cooldown
     */
    async detectKeywordInOutput(keyword) {
        // Check cooldown period - IGNORE ALL DETECTIONS for 60 seconds after last detection
        const timeSinceLastDetection = Date.now() - this.lastDetectionTime;
        if (timeSinceLastDetection < this.detectionCooldown) {
            console.log(`⏳ In cooldown period (${Math.round((this.detectionCooldown - timeSinceLastDetection) / 1000)}s remaining)`);
            return false;
        }
        ...
        if (detected) {
            ...
            console.log(`⏱️  Detection cooldown started (${this.detectionCooldown / 1000}s)`);
        }
    ...
    /**
     * Override executeChainAction to emit keyword_detected instead of executing chains
     * This fixes the hanging issue where chain execution interferes with promise resolution
     */
    async executeChainAction(chainConfig) {
        const { keyword, chainIndex } = chainConfig;
        
        console.log(`🎯 KEYWORD DETECTED: "${keyword}"`);
        console.log(`📋 Emitting keyword_detected event and stopping monitor`);
        
        // Emit the keyword_detected event that the e2e executor is waiting for
        ...
        // Stop monitoring immediately - we found what we were looking for
        this.stop();
        
        // Note: We deliberately skip chain execution logic from parent class
        // because we only want keyword detection, not instruction sending
    }
}
