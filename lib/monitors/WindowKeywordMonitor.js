...
class WindowKeywordMonitor extends ChainKeywordMonitor {
    ...
    async checkOutput() {
        ...
        try {
            ...
            if (hasNewContent) {
                // Content changed - reset position tracking since buffer is completely replaced
    ...
    async readFromInstance() {
        try {
            // Restore original working buffer size that worked flawlessly
            // 200 lines captures enough content even with long Claude responses
