...
export class QueueMetrics {
    ...
    /**
     * Generate history report for specified time range
     */
    generateHistoryReport(timeRange = '24h') {
        const timeRangeMs = this.parseTimeRange(timeRange);
        const cutoffTime = Date.now() - timeRangeMs;
        
        const recentEvents = this.metrics.cleanupEvents.filter(
        ...
        const recentQueueSizes = this.metrics.queueSizeHistory.filter(
        ...
        return {
            ...
            events: {
                cleanups: recentEvents.length,
                errors: recentErrors.length,
                queueObservations: recentQueueSizes.length
            },
            details: {
                cleanupEvents: recentEvents,
                errorEvents: recentErrors,
                queueSizeHistory: recentQueueSizes
            }
        };
    }
