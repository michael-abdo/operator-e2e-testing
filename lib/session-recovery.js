    ...
    async recoverClaudeWindow(windowName) {
        ...
        try {
            ...
            if (!windows.includes(windowName)) {
                ...
                execSync(`tmux send-keys -t ${windowName} 'claude' Enter`);
            ...
            const windowContent = execSync(`tmux capture-pane -t ${windowName} -p | tail -5`).toString();
            ...
            if (windowContent.includes('error') || windowContent.includes('Error')) {
                ...
                execSync(`tmux kill-window -t ${windowName} 2>/dev/null || true`);
                execSync(`tmux new-window -n ${windowName}`);
                execSync(`tmux send-keys -t ${windowName} 'claude' Enter`);
